    ...
    <>
      ...
      <aside className={`fixed bottom-0 right-0 top-0 z-[70] w-72 transform bg-white shadow-2xl transition-transform duration-300 ease-in-out lg:hidden ${isMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
          ...
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
            <span className="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-700">Admin Menu</span>
          ...
          <div className="space-y-2 border-t border-slate-100 p-3">
            <a href="/" target="_blank" rel="noreferrer" className="flex items-center gap-3 rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
              <Globe className="h-4 w-4" /> Public Site
            </a>
            <button onClick={handleLogout} className="flex w-full items-center gap-3 rounded-lg border border-rose-100 bg-rose-50 px-3 py-2.5 text-sm font-bold text-rose-600 transition hover:bg-rose-100">
      ...
      <header className="sticky top-0 z-50 border-b border-white/60 bg-white/80 shadow-[0_4px_20px_rgba(15,23,42,0.03)] backdrop-blur-md">
        <div className="mx-auto flex max-w-[100rem] items-center justify-between px-4 py-1.5 sm:px-5 sm:py-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[0.12em] text-emerald-700 sm:text-[10px]">
                <ShieldCheck className="h-3 w-3" /> Admin
              </div>
              <h1 className="mt-0.5 text-sm font-semibold tracking-tight text-slate-900 sm:text-base">IIITians Network Admin</h1>
            </div>
          </div>
          ...
          <div className="hidden lg:flex lg:items-center lg:gap-4">
            <nav className="flex items-center gap-1">
              {adminLinks.map((link) => (
                <Link key={link.href} href={link.href}
                  className={`whitespace-nowrap rounded-full px-2.5 py-1.5 text-[11px] font-medium transition ${pathname.startsWith(link.href) ? "bg-slate-900 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"}`}>
                  {link.label}
                </Link>
              ))}
            </nav>
        ...
        <div className="relative z-10 flex min-h-screen flex-col">
          ...
          <footer className="mt-auto border-t border-slate-800 bg-slate-950 py-5">
            <div className="mx-auto max-w-7xl px-4 sm:px-6">
              <div className="hidden grid-cols-3 gap-6 lg:grid">
                <div className="max-w-xs">
                  ...
                  <p className="mt-2 text-[12px] leading-5 text-slate-400">
                    Protected session active. Ensure verification before approving network members. Internal data is encrypted.
                  </p>
                </div>

                <div>
                  <h3 className="text-sm font-semibold uppercase tracking-wider text-slate-500">Quick Tools</h3>
                  <ul className="mt-3 flex flex-col gap-2 text-sm text-slate-300">
                    <li>
                      <Link href="/admin/guide" className="transition hover:text-emerald-400">
                        Admin Guide
                      </Link>
                    </li>
                    <li>
                      <Link href="/legacy" className="transition hover:text-emerald-400">
                ...
                <div className="text-right">
                  <h3 className="text-sm font-semibold uppercase tracking-wider text-slate-500">Support</h3>
                  <p className="mt-3 text-[12px] text-slate-400">
                    Internal Portal &bull; &copy; {new Date().getFullYear()}
                  </p>
                  <div className="mt-4 flex justify-end gap-4">
                    <Link href="/" className="text-xs font-medium text-slate-500 hover:text-white">
                      Main Site
                    </Link>
                    <Link href="/contact" className="text-xs font-medium text-slate-500 hover:text-white">
                      Helpdesk
                    </Link>
                  </div>
                </div>
              ...
              <div className="mt-5 flex flex-col items-center justify-between gap-3 border-t border-slate-800 pt-5 sm:flex-row lg:mt-6">
