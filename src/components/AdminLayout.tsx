          ...
          <nav className="flex-1 space-y-1 overflow-y-auto p-4 custom-scrollbar">
            {adminLinks.map((link) => (
              <Link key={link.href} href={link.href}
                className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition ${pathname.startsWith(link.href) ? "bg-slate-900 text-white shadow-lg shadow-slate-900/10" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`}>
                {link.label}
              </Link>
            ))}
          </nav>
        ...
        <div className="mx-auto flex max-w-[100rem] items-center justify-between px-4 py-2 sm:px-6 sm:py-2.5">
          ...
          <div className="hidden lg:flex lg:items-center lg:gap-4">
            <nav className="flex items-center gap-1">
              {adminLinks.map((link) => (
                <Link key={link.href} href={link.href}
                  className={`whitespace-nowrap rounded-full px-2.5 py-1.5 text-[12px] font-medium transition ${pathname.startsWith(link.href) ? "bg-slate-900 text-white shadow-sm" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"}`}>
                  {link.label}
                </Link>
              ))}
            </nav>
